...
public class Minimisation{
            ...
            for(int i=0; i<this.nConstraints; i++){
                ...
                switch(penaltyCheck[i]){
                    case -1: if(param[k]<constraints[i]){
                                funcVal = tempFunctVal + this.penaltyWeight*this.square(constraints[i]-param[k]);
                                test=false;
                             }
                             break;
                    case 0: if(param[k]<constraints[i]*(1.0-this.constraintTolerance)){
                                funcVal = tempFunctVal + this.penaltyWeight*this.square(constraints[i]*(1.0-this.constraintTolerance)-param[k]);
                                test=false;
                             }
                             if(param[k]>constraints[i]*(1.0+this.constraintTolerance)){
                                funcVal = tempFunctVal + this.penaltyWeight*this.square(param[k]-constraints[i]*(1.0+this.constraintTolerance));
                                test=false;
                             }
                             break;
                    case 1:  if(param[k]>constraints[i]){
                                funcVal = tempFunctVal + this.penaltyWeight*this.square(param[k]-constraints[i]);
	                            test=false;
	                         }
	                         break;
                }
            ...
            for(int i=0; i<this.nSumConstraints; i++){
                ...
                switch(this.sumPenaltyCheck[i]){
                    case -1: if(sumPenaltySum<sumConstraints[i]){
                                funcVal = tempFunctVal + this.penaltyWeight*this.square(sumConstraints[i]-sumPenaltySum);
                                test=false;
                             }
                             break;
                    case 0: if(sumPenaltySum<sumConstraints[i]*(1.0-this.constraintTolerance)){
                                funcVal = tempFunctVal + this.penaltyWeight*this.square(sumConstraints[i]*(1.0-this.constraintTolerance)-sumPenaltySum);
                                test=false;
                             }
                             if(sumPenaltySum>sumConstraints[i]*(1.0+this.constraintTolerance)){
                                funcVal = tempFunctVal + this.penaltyWeight*this.square(sumPenaltySum-sumConstraints[i]*(1.0+this.constraintTolerance));
                                test=false;
                             }
                             break;
                    case 1:  if(sumPenaltySum>sumConstraints[i]){
                                funcVal = tempFunctVal + this.penaltyWeight*this.square(sumPenaltySum-sumConstraints[i]);
	                            test=false;
                             }
                             break;
                }
	...
	public void addConstraint(int[] paramIndices, int[] plusOrMinus, int conDir, double constraint){
		miArray am = new miArray(plusOrMinus);
	...
	public void addConstraint(int[] paramIndices, double[] plusOrMinus, int conDir, double constraint){
		...
		this.sumPenalties.add(plusOrMinus);
 	    ...
 	    miArray am = new miArray(paramIndices);
	...
	public void print(String filename){
        ...
        for(int i=0; i<this.nParam; i++){
            ...
            fout.printtab(this.truncate(paramValue[i],this.prec), this.field);
             fout.printtab(this.truncate(this.startH[i],this.prec), this.field);
            fout.println(this.truncate(this.step[i],this.prec));
        }
        fout.println();
        ...
        fout.println(this.truncate(this.simplexSd, this.prec));
 ...
 // MANTISSA ROUNDING (TRUNCATING)
    ...
    public static double truncate(double xDouble, int trunc){
        ...
        if(!Minimisation.isNaN(xDouble)){
            if(!Minimisation.isPlusInfinity(xDouble)){
                if(!Minimisation.isMinusInfinity(xDouble)){
                    if(xDouble!=0.0D){
