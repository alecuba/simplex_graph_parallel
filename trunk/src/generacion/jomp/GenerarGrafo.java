...
public class GenerarGrafo {
	...
	public boolean generar(int minSecciones,int maxSecciones,int minCruces,int maxCruces,int minEntradas,int maxEntradas){
		...
		int largo=(int)Math.rint((double)minSecciones/(double)minCruces);
		if(largo>anchoTabla) anchoTabla=largo;
		if((anchoTabla*maxCruces)>maxSecciones){
			System.out.println("No hay combinacion posible debes poner maxSecciones a:"+(anchoTabla*maxCruces));
			retorno=false;
		}else{
			if(maxCruces*(anchoTabla-1)<maxSecciones) maxSecciones=(maxCruces*(anchoTabla-1));
			...
			algoritmoGeneraSecciones(minSecciones,maxSecciones,minCruces,maxCruces);
	...
	private void algoritmoGeneraSecciones(int nseccionesMinGrafo,int nseccionesMaxGrafo, int minCruces, int maxCruces){
		...
		while(ncrucesEnTotal<nseccionesMinGrafo&&!visitadosTodaslasIes()){
...
// OMP PARALLEL BLOCK BEGINS
{
  ...
  __omp_Object0.maxCruces = maxCruces;
  __omp_Object0.minCruces = minCruces;
  __omp_Object0.nseccionesMaxGrafo = nseccionesMaxGrafo;
  __omp_Object0.nseccionesMinGrafo = nseccionesMinGrafo;
  ...
  maxCruces = __omp_Object0.maxCruces;
  minCruces = __omp_Object0.minCruces;
  nseccionesMaxGrafo = __omp_Object0.nseccionesMaxGrafo;
  nseccionesMinGrafo = __omp_Object0.nseccionesMinGrafo;
	...
	public void insertaGrafoCQL(boolean recreaBD){
	      ...
	      try{
	    	  ...
	    	  if(recreaBD){
	    		  ...
	    		  gestor.getCassandraSession().execute("CREATE TABLE Bd.vertices (vertA int, vertB int, idseccion int PRIMARY KEY)");
	      ...
	      PreparedStatement psExtremos = gestor.getCassandraSession().prepare("INSERT INTO Bd.vertices (vertA,vertB,idseccion) VALUES (?, ?, ?)");
...
private class __omp_Class0 extends jomp.runtime.BusyTask {
  ...
  // reduction variables, init to default
    // OMP USER CODE BEGINS
			...
			while((ncrucesLineaActual < ncrucesLineaActualMax) && !visitados && (ncrucesEnTotal<nseccionesMaxGrafo || ncrucesEnTotal<nseccionesMinGrafo)){
